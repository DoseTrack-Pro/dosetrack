...
double calcReconVolume(double peptideMg, double desiredDoseMcg, int targetDoses) {
  if (peptideMg <= 0 || desiredDoseMcg <= 0 || targetDoses <= 0) return 0;
  final doseIu = desiredDoseMcg / (peptideMg * 1000) * 100; // IU per dose at 1mL
...
bool isScheduledOnDate(Device device, DateTime date) {
  switch (device.schedule) {
    ...
    case DoseSchedule.everyOtherDay:
      return date.difference(device.createdAt).inDays % 2 == 0;
    case DoseSchedule.twiceWeekly:
    case DoseSchedule.onceWeekly:
    case DoseSchedule.custom:
...
bool isScheduledToday(Device device) => isScheduledOnDate(device, DateTime.now());
...
int daysUntilExpiry(Device device) {
  ...
  final expiry = reconDate.add(const Duration(days: 28));
...
int calcAdherence(List<Device> activeDevices, List<DoseLog> logs) {
  ...
  final scheduledDevices = activeDevices.where((d) =>
    d.schedule != DoseSchedule.custom || effectiveScheduleDays(d).isNotEmpty,
  ).toList();
