...
class _ReconCalculatorModalState extends State<ReconCalculatorModal> {
  ...
  @override
  void dispose() {
    for (final c in [_mgCtrl, _mlCtrl, _mcgCtrl, _mgWCtrl, _mcgWCtrl, _dosesCtrl]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  bool get _drawReady => _drawMg > 0 && _drawMl > 0 && _drawMcg > 0 && _drawIu > 0;
  bool get _waterReady => _waterMg > 0 && _waterMcg > 0 && _waterDoses > 0 && _waterMl > 0;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(children: [
                const Icon(Icons.calculate_outlined, color: AppColors.teal, size: 22),
                const SizedBox(width: 10),
                Text('Reconstitution Calculator',
                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: context.clrText)),
              ]),
              ...
              if (_mode == _CalcMode.draw) ...[
                Text('INPUTS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                    color: context.clrTextSub, letterSpacing: 0.7)),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _CalcField(label: 'Peptide (mg)', hint: 'e.g. 5', controller: _mgCtrl, onChanged: (_) => setState(() {}))),
                  const SizedBox(width: 12),
                  Expanded(child: _CalcField(label: 'BAC water (mL)', hint: 'e.g. 2', controller: _mlCtrl, onChanged: (_) => setState(() {}))),
                ]),
                _CalcField(label: 'Desired dose (mcg)', hint: 'e.g. 250', controller: _mcgCtrl, onChanged: (_) => setState(() {})),
                const SizedBox(height: 16),

                if (_drawReady) ...[
                  _ResultCard(
                    items: [
                      _ResultItem(label: 'Draw volume', value: '${_drawIu.toStringAsFixed(1)} IU', primary: true),
                      _ResultItem(label: 'Total doses from vial', value: '$_drawTotal doses'),
                    ],
                  ),
                ] else
                  _PlaceholderCard(text: 'Fill in all three fields above to see results'),
              ],
              ...
              if (_mode == _CalcMode.water) ...[
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _CalcField(label: 'Peptide (mg)', hint: 'e.g. 5', controller: _mgWCtrl, onChanged: (_) => setState(() {}))),
                  const SizedBox(width: 12),
                  Expanded(child: _CalcField(label: 'Desired dose (mcg)', hint: 'e.g. 250', controller: _mcgWCtrl, onChanged: (_) => setState(() {}))),
                ]),
                _CalcField(label: 'Doses you want from vial', hint: 'e.g. 20', controller: _dosesCtrl,
                ...
                if (_waterReady) ...[
                  _ResultCard(
                    items: [
                      _ResultItem(label: 'Add this much BAC water', value: '${_waterMl.toStringAsFixed(2)} mL', primary: true),
                      _ResultItem(label: 'Draw volume per dose', value: '${_waterIu.toStringAsFixed(1)} IU'),
                    ],
                  ),
                ] else
                  _PlaceholderCard(text: 'Fill in all three fields above to see results'),
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Formula reference', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                      color: context.clrTextSub, letterSpacing: 0.5)),
                  const SizedBox(height: 6),
                  Text('IU = (mcg ÷ (mg × 1000)) × mL × 100',
                      style: TextStyle(fontSize: 12, color: context.clrTextHint, fontFamily: 'Courier New')),
                  const SizedBox(height: 4),
                  Text('IU markings on a U-100 insulin syringe represent μL of volume.',
                      style: TextStyle(fontSize: 11, color: context.clrTextHint, height: 1.4)),
                ]),
              ),
            ],
          ),
    ...
    child: GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(icon, size: 14, color: active ? Colors.white : context.clrTextSub),
      ...
      children: items.asMap().entries.map((e) {
        ...
        return Expanded(child: Row(children: [
          Expanded(child: Column(children: [
            Text(item.label, style: TextStyle(fontSize: 11, color: AppColors.tealDeep), textAlign: TextAlign.center),
            const SizedBox(height: 6),
            FittedBox(fit: BoxFit.scaleDown, child: Text(item.value, style: TextStyle(
          ...
          if (!isLast) Container(width: 0.5, height: 44, color: AppColors.tealMid,
...
class _ResultItem {
  ...
  final bool primary;
  const _ResultItem({required this.label, required this.value, this.primary = false});
}
...
class _PlaceholderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Center(child: Text(text, style: TextStyle(fontSize: 13, color: context.clrTextHint), textAlign: TextAlign.center)),
  );
}
