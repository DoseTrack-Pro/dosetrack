      ...
      child: SafeArea(
        ...
        child: Padding(
          padding: EdgeInsets.fromLTRB(
              20, 16, 20, 28 + MediaQuery.of(context).viewInsets.bottom),
          ...
          child: Stack(
            ...
            children: [
              ...
              AnimatedBuilder(
                  animation: _ring1,
                  builder: (_, __) =>
                      _PulseRing(progress: _ring1.value, size: 120)),
              AnimatedBuilder(
                  animation: _ring2,
                  builder: (_, __) =>
                      _PulseRing(progress: _ring2.value, size: 90)),
              // Countdown arc
              CustomPaint(
                  size: const Size(160, 160),
                  painter: _CountdownArcPainter(
                    progress:
                        _maxCountdown > 0 ? _countdown / _maxCountdown : 0,
                    color: timerColor,
    ...
    return Column(
      children: [
        Container(
          ...
          child:
              const Icon(Icons.check_rounded, color: AppColors.teal, size: 36),
        ),
        ...
        Text('NFC Tag Detected',
            style: TextStyle(
                ...
                color: context.clrText)),
        const SizedBox(height: 6),
        Text('Review and confirm the matched compound',
            textAlign: TextAlign.center,
        ...
        Container(
          ...
          child: Row(children: [
            DoseRing(
                remaining: d.remainingDoses, total: d.totalDoses, size: 70),
            ...
            Expanded(
                child: Column(
                  ...
                  Text(
                      '${d.schedule.label}  ·  ${d.desiredDoseMcg.toStringAsFixed(0)}mcg / ${d.doseVolumeIu.toStringAsFixed(1)}IU',
                      style: TextStyle(fontSize: 12, color: context.clrTextSub),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis),
                ])),
        ...
        if (!isDepleted) ...[
          ...
          BodySitePicker(
            ...
            recentCounts:
                siteUsageCounts(ref.read(doseLogsProvider), _detected!.id),
        ...
        if (isDepleted) ...[
          ...
          TextButton(
              onPressed: () => Navigator.pop(context),
              child:
                  Text('Cancel', style: TextStyle(color: context.clrTextSub))),
        ],
      ],
    );
  }
    ...
    return Column(
      children: [
        Container(
          ...
          height: 72,
          decoration:
              BoxDecoration(color: context.clrRedBg, shape: BoxShape.circle),
        ...
        Text(
          _error.isNotEmpty
              ? _error
              : 'The scan timed out without finding an NFC tag.',
          textAlign: TextAlign.center,
          style:
        ...
        if (nfcDevices.isNotEmpty) ...[
          Align(
              alignment: Alignment.centerLeft,
              child: Text('LOG MANUALLY INSTEAD',
                  style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w700,
                      color: context.clrTextSub,
                      letterSpacing: 0.6))),
          const SizedBox(height: 8),
          ...nfcDevices.where((d) => d.remainingDoses > 0).map((d) =>
              GestureDetector(
                onTap: () {
                  Navigator.pop(context);
                  widget.onManualLog(d);
                },
                child: Container(
                  margin: const EdgeInsets.only(bottom: 6),
                  padding:
